...
CONFIG = {
    ...
    "experiment_name": os.getenv("EXPERIMENT_NAME", "MLflow Experiment")
}
...
if _DAGSHUB_AVAILABLE:
    try:
        dagshub.init(repo_owner=CONFIG.get("dagshub_repo_owner"), repo_name=CONFIG.get("dagshub_repo_name"), mlflow=True)
    except Exception as e:
        print(f"dagshub init failed: {e}. Proceeding without dagshub.")
else:
    print("dagshub not available; proceeding without remote tracking.")

# Ensure an experiment name is set locally even if dagshub isn't used
try:
    mlflow.set_experiment(CONFIG.get("experiment_name") or "Default Experiment")
except Exception:
    pass
